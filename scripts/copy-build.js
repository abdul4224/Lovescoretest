...
// 4. Copy dist/tool/ -> root tool/
const distTool = path.join(distDir, 'tool');
const rootTool = path.join(rootDir, 'tool');
if (fs.existsSync(distTool)) {
  fs.cpSync(distTool, rootTool, { recursive: true });
  const toolSlugs = fs.readdirSync(rootTool);
  console.log(`✓ Copied ${toolSlugs.length} static tool routes -> root tool/`);
}

// 5. Copy dist/catalog/ -> root catalog/
const distCatalog = path.join(distDir, 'catalog');
const rootCatalog = path.join(rootDir, 'catalog');
if (fs.existsSync(distCatalog)) {
  fs.cpSync(distCatalog, rootCatalog, { recursive: true });
  console.log('✓ Copied static catalog route -> root catalog/');
}
